...
def test_does_not_enter_on_a_close_that_is_only_inside_its_own_bar():
    # Regression test for the self-inclusion bug: the channel from
    # Donchian.update() must come from the PRIOR bars, not one that already
    # contains this bar's own high -- otherwise a breakout can never fire.
    strategy = DonchianBreakoutStrategy("BTC/USDT", period=3, position_pct=0.5)
    broker = PaperBroker(cash=1000)

    # Flat range for 3 bars, then a bar whose high/low/close are all equal
    # (100) -- if the channel included this bar, close >= upper would be
    # trivially true every single bar.
    for i in range(3):
...
def test_exits_on_a_breakdown_below_the_prior_channel():
    ...
    assert len(broker.positions) == 0
    assert any(t.get("reason") == "donchian_breakdown" for t in broker.trade_log)
